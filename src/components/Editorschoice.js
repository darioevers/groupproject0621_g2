import React from "react";

function Editorschoice() {
  return (
    <div className="editor-choice">
      <div className="editor-heading">
        <h2>
          EDITORS CHOICE <span>//</span>
        </h2>
      </div>
      <div className="editor-items-container">
        <div className="editor-items">
          <div className="editor-item">
            <div className="editor-item-image">
              <img src="https://lh6.googleusercontent.com/proxy/FOzYtX-jc1HAda1lek_cw6G5jabe3ApZFywHfGbltF1Znr4Q8rHyxnvje-J2IH2oRZmnBQd-SE-Gfvcf8kjQCJN7osMPI_YFT1yCirSVYIkfZIKAKZen7nGv0Teu5CWPHVcvUyaoVdM6=s0-d"></img>
            </div>
            <div className="editor-item-heading">
              <a
                href="https://hetnice.blogspot.com/2019/04/gif-wallpaper-tech.html"
                target="_blank"
              >
                <h3>Definition of gif wallpaper tech</h3>
              </a>
            </div>
            <div className="author-date">
              <span>John Walker</span> <span> / </span>
              <span>2021-06-30</span>{" "}
            </div>
          </div>
          <div className="editor-item">
            <div className="editor-item-image">
              <img src="https://cdn.substack.com/image/fetch/w_1456,c_limit,f_auto,q_auto:good,fl_lossy/https%3A%2F%2Fbucketeer-e05bbc84-baa3-437e-9518-adb32be77984.s3.amazonaws.com%2Fpublic%2Fimages%2Fb658ff68-716a-407f-8d01-44d51fb752a6_500x281.gif"></img>
            </div>
            <div className="editor-item-heading">
              <a
                href="https://classicalalternative.substack.com/p/lessons-from-aubrey-bergauer"
                target="_blank"
              >
                <h3>Lessons from Aubrey Bergauer</h3>
              </a>
            </div>
            <div className="author-date">
<<<<<<< HEAD
              <span>AUTHOR</span> <span> / </span>
              <span>DATE</span>
=======
              <span>Aubrey Bergauer</span> <span> / </span>
              <span>2021-05-20</span>{" "}
>>>>>>> fabcc649add614b3720884d2b4523d082e9df053
            </div>
          </div>
          <div className="editor-item">
            <div className="editor-item-image">
              <img src="https://i.gifer.com/7RMy.gif"></img>
            </div>
            <div className="editor-item-heading">
              <a
                href="https://monophy.com/gifs/tooz-tech-technologies-NOEG6shRi2taYK2olX"
                target="_blank"
              >
                <h3>Glasses Technology</h3>
              </a>
            </div>
            <div className="author-date">
              <span>Tooz Tech</span> <span> / </span>
              <span>2021-06-17</span>{" "}
            </div>
          </div>
          <div className="editor-item">
            <div className="editor-item-image">
              <img src="https://api.time.com/wp-content/uploads/2021/01/Stock-Tech-Illustration-Big-Tech.gif"></img>
            </div>
            <div className="editor-item-heading">
              <a
                href="https://time.com/5934393/facebook-oversight-board-big-tech-future/"
                target="_blank"
              >
                <h3>Facebook’s New Oversight Board</h3>
              </a>
            </div>
            <div className="author-date">
              <span>www.time.com</span> <span> / </span>
              <span>2021-04-30</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default Editorschoice;
